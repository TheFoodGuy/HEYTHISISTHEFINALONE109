...
bool TCPServerSocket::initializeSocket ( ) // Initialize server socket
{
	...
	cout << "error was in creation of socket" <<endl;
	return false;
	}
        ...
        int result = setsockopt(sock,
                          ...
                          (char *) &flag,
                          sizeof(int));
        if ( result < 0 ) //on success for result, it should return 0, else -1 
        //it returns -1 only if there is an error or some sort. 
        {
	       close (sock); //guess to close the original one 
      	       sock = -1;
       	       return false;
    	}
        ...
        if ( result < 0 )
 	{
        ...
        if (listen(sock, backlog) == -1) //same idea as bind, we would bind. 
        {
		close (sock);
		sock = -1;
		return false;
...
TCPSocket * TCPServerSocket::getConnection (int timeoutSec, int timeoutMilli,int readBufferSize,int writeBufferSize )
{
	...
	if (timeoutSec==0 && timeoutMilli == 0 )// Blocking mode //meaning it'll be at an infinite mode'
	{
		...
		if (FD_ISSET(sock, &fds)) // if sock is changed 
		{
			...
			newsock = accept(sock, (struct sockaddr *)&clientAddr,&sin_size); //same idea as the first if statement but with a timer to wait for the client or server. 
		}
	}
        ...
        {   // Print the error and return NULL
            ...
            return NULL;
                ...
                return tcpSocket; //creates the new socket for the connection basically if everything is great 
        }
